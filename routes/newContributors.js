...
const NewContributor = require("../models/NewContributor");
//const verifyToken = require("../verifyToken");


//STORE NEWCONTRIBUTOR OBJECT

router.post("/:org/:repo/:year?/:month?", async (req, res) => {
  ...
  const month = req.params.month;
    try {
      ...
      commitsField.forEach((obj) => {
        ...
        const newContributor = new NewContributor(outputObj)
...
//GET SINGLE NEWCONTRIBUTOR OBJECT
...
//GET ALL NEWCONTRIBUTORS OBJECT

router.get("/all", async(req,res)=>{
  try{
    const newContributors = await NewContributor.find();
    res.status(200).json(newContributors.reverse());
  }
  catch(err){
    res.status(500).json(err);
  }
